...
class MergingMethod:
    ...
    def finer_graned(self, models_to_merge, merged_model):
        with torch.no_grad():
            base = merged_model
    ...
    def rankmean(self, models_to_merge, merged_model):
        with torch.no_grad():
            base = merged_model
            model1 = models_to_merge[0]
            model2 = models_to_merge[1]
            ...
            num_hidden_layers = model1.config.num_hidden_layers
            print(num_hidden_layers)

            m1_sums = [ 0 for i in range(num_hidden_layers) ]
            m2_sums = [ 0 for i in range(num_hidden_layers) ]
    ...
    def reversed(self, models_to_merge, merged_model):
        with torch.no_grad():
            base = merged_model
            model1 = models_to_merge[0]
            model2 = models_to_merge[1]
            ...
            for ((n,v1),(_,v2), (_,v3)) in it:
                ...
                if layer_name[1] != "layers":
                    ...
                    layer_num = int(layer_name[2])
                    p = layers_rank_m2[layer_num] / (layers_rank_m1[layer_num] + layers_rank_m2[layer_num]) # REVERSED for 2 models
    ...
    def merging_models(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list, trainers: list = None, scaling_coefficient: float = 1.0,
        ...
        if self.merging_method_name == "average_merging":
        ...
        elif self.merging_method_name == "finer_grained":
            merged_params = self.finer_graned(models_to_merge=models_to_merge, merged_model=merged_model)
        elif self.merging_method_name == "rankmean":
            merged_params = self.rankmean(models_to_merge=models_to_merge, merged_model=merged_model)
        elif self.merging_method_name == "reversed":
            merged_params = self.reversed(models_to_merge=models_to_merge, merged_model=merged_model)
            ...
            if mask_apply_method == "average_merging":
                ...
                merged_params = self.finer_graned(models_to_merge=new_models_to_merge, merged_model=merged_model)
